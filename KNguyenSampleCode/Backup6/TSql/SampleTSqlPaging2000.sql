...
/* paging for sql 2000 -- create table in memory to store sample data */
...
declare @TempIndex table (
	[RowNum] int identity, --row num
	[UniqueId] int --unique id of the result which inner joins other tables in a 1:1 relationship (the row num cannot be duplicated)
)

declare @intReadUpperBound int
set @intReadUpperBound = (@intPageIndex-1) * @intPageSize + @intPageSize
set rowcount @intReadUpperBound --overcapture search space (captures rows before target set -- this is the drawback for sql2000, but optimize data copy by only storing index value)

insert into @TempIndex ([UniqueId])
	select [ProductId]
	from @Product
...
set rowcount 0 --reset to infinite upperbound

select * from (
	select [ProductId], [Name], [Description], [Sku], [Price], [RowNum]
	from @TempIndex idx
	inner join @Product p
	on p.[ProductId]=idx.[UniqueId]
) t
where [RowNum] > (@intPageIndex-1)*@intPageSize
order by [RowNum]
